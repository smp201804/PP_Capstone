...
#create a combined field for start date and start time, start_dt, and end date / end time, end_dt.
...
I calculated the mean by Type  per below.  It’s not surprising that U1 has the highest mean as a U1 is an outage for which the generating unit must be shut down immediately
...
I created box-plot graphs by type, in which the x axis was outage type, Type, and the y axis was time to repair, Time_To_Repair, for each data frame.  I also created stat summary graphs, x axis was year, y axis was type, and faceted by outage type.  Both show distributions with long right tails and high range outliers.
